...
actor SuperHeros{
  ...
  public query func getHero(superHeroId: SuperHeroId) : async ?SuperHero {
    let result = Trie.find(superheroes, key(superHeroId), Nat32.equal);
    result
  };

  public func updateHero(superHeroId: SuperHeroId, superhero: SuperHero): async Bool {
    let result = Trie.find(superheroes, key(superHeroId), Nat32.equal);
  ...
  public func deleteHero(superHeroId: SuperHeroId): async Bool {
    let result = Trie.find(superheroes, key(superHeroId), Nat32.equal);
    let exists = Option.isSome(result);
    if(exists) {
      superheroes := Trie.replace(
